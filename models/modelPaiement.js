...
module.exports = {
    recup_idMissionSansDistance: function (callback) {
        ...
        var sql = 'SELECT sa_nom, sa_prenom, DATE_FORMAT(mi_dateDebut, "%d/%m/%Y") as mi_dateDebut ,DATE_FORMAT(mi_dateFin, "%d/%m/%Y") as mi_dateFin, mi_lieuDepart, mi_idcommune, co_nom from mission, salarie, communes WHERE co_id = mi_idcommune and mi_id NOT IN (SELECT mi_id from mission, distance,salarie WHERE sa_id = mi_idsalarie AND ((mi_lieuDepart = di_idComDepart AND mi_idcommune = di_idComArrive) OR (mi_lieuDepart = di_idComArrive AND mi_idcommune = di_idComDepart))) and sa_id = mi_idsalarie ';
    ...
    afficher_paiement: function (callback) {
       ...
       var sql = 'SELECT *,  DATE_FORMAT(mi_dateDebut, "%d/%m/%Y") as mi_dateDebut ,DATE_FORMAT(mi_dateFin, "%d/%m/%Y") as mi_dateFin from mission, distance,salarie, communes WHERE co_id = mi_idcommune AND sa_id = mi_idsalarie AND ((mi_lieuDepart = di_idComDepart AND mi_idcommune = di_idComArrive) OR (mi_lieuDepart = di_idComArrive AND mi_idcommune = di_idComDepart))'
        ...
        var sql4 = ' SELECT *,  DATE_FORMAT(mi_dateDebut, "%d/%m/%Y") as mi_dateDebut ,DATE_FORMAT(mi_dateFin, "%d/%m/%Y") as mi_dateFin from mission, distance,salarie, communes WHERE co_id = mi_lieuDepart AND sa_id = mi_idsalarie AND ((mi_lieuDepart = di_idComDepart AND mi_idcommune = di_idComArrive) OR (mi_lieuDepart = di_idComArrive AND mi_idcommune = di_idComDepart))'
